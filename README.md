...
当前已在 RTX PRO 6000 96GB 上实测 vLLM 0.25.1 ModelOpt NVFP4/Marlin 专家数据面：20,480 个 checksummed SSD runtime objects、128-slot actual Top-22 blocking load、Level 1/2 释放恢复和 host-staged RC RDMA get/put。75→71 跨机 volatile pool 已搬运 15.859GB 真实 expert profile；真实 route predictor、Mamba transaction、RDMA-to-GPU-slot 与 DGX Spark UMA 实验仍待完成。
...
- **Route-Preserving Elastic Expert Residency**：保持 Nemotron 原始 Top-22 路由，只动态收缩 routed-expert physical slots；预测 miss 必须加载正确 expert。这是路由不变式，不等于已证明跨请求逐 token 输出等价。
...
- **DGX Spark RDMA fallback**：能力探测禁止在 GB10 上声明 GPUDirect RDMA。Durable RC store 提供 checksum/manifest/落盘；高性能 `pllm-rdma-pool` 在 71 预注册 volatile host MR，由计算节点执行 one-sided PUT/GET，远端 CPU 和文件系统不进入热路径。当前 GET 仍先写本地 `.pllmex`，未直入 CUDA/UMA slot。
...
`run_vllm.sh` 使用 NVFP4 + Marlin、FP8 KV、32K context、最多 2 个请求、HiberCache 与 safetensors loader。EER 启动脚本默认 45% GPU 内存上限，避免 vLLM 把释放的 expert 空间重新分配给 KV cache。模型端口只绑定 `127.0.0.1`；vLLM 的开发控制端点不得暴露到局域网。
...
该路径强制 vLLM 0.25.1、ModelOpt NVFP4、Marlin modular backend、lazy safetensors 与 eager execution。当前实测导出 20,480 objects、63,435,912,912 bytes，约 308.85 秒。128 slots 为保持 fused Top-22 精确执行，自动把 `max_num_batched_tokens` 限制为 5；该限制显著降低吞吐。
...
真实 expert profile 的 volatile pool 使用 `rdma_bridge/build/pllm-rdma-pool` 与 `scripts/run_rdma_memory_shards.py`，完整命令见 `rdma_bridge/README.md`。已有跨机 v1 结果为 15.859GB PUT 2.545s（49.85Gb/s wall）、GET 到本地文件 4.012s（31.62Gb/s wall）。Selective-signaling/inline-header v2 尚未跨机复测，不用本机 loopback 数据替代。
