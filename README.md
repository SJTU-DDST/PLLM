...
当前已在 RTX PRO 6000 96GB 上实测 vLLM 0.25.1 ModelOpt NVFP4/Marlin 专家数据面：20,480 个 checksummed SSD runtime objects、128-slot actual Top-22 blocking load、Level 1/2 释放恢复和 host-staged RC RDMA get/put。真实 route predictor、Mamba transaction、双机吞吐与 DGX Spark UMA 实验仍待完成。
...
- 完整回归为 `55 passed`；compileall、shell syntax 和 CMake 构建通过。
...
- `mlx5_0` 真实完成 20MiB RC RDMA PUT/GET，三端 SHA-256 一致；未测独立双机网络 bandwidth，也不声明 GDR。
