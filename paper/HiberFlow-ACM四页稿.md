...
> 研究设计与系统原型稿。本文明确区分静态测量、已完成控制面、已实现未运行的数据面和待运行真实模型实验。当前 GPU 正在执行其他实验，本文不报告 Nemotron 真实吞吐、释放时间或恢复时间。
...
深度休眠阶段沿用 HiberFlow 的事务式状态协议：在 token 边界提交 Attention KV、Mamba recurrent state、采样器和输出 ledger，直接丢弃已有 canonical checkpoint 的不可变权重，并从 NVMe 或无 GPUDirect RDMA 的 host-staged RDMA 路径恢复。本文实现了只读取 17 个 safetensors header 的 catalog；它确认 74.783GiB tensor 中严格匹配 `experts.<id>` 的 physical expert objects 为 59.063GiB，非 routed 权重为 15.720GiB。每层投影 64/512 个 slots 时，理论常驻权重约 23.103GiB。本文给出算法、正确性边界、vLLM 实现路径及可证伪实验，而不把 header projection 或合成 trace 写成运行时结果。
...
Spark 不支持 GPUDirect RDMA、nvidia-peermem、DMA-BUF 或 GDRCopy；NVIDIA 建议 verbs 应用使用 `cudaHostAlloc` 后注册 MR [11]。远端 expert 传输必须走 ConnectX-7 到 host staging，再进入可计算的 UMA slot。当前独立 bridge 先把 staging bytes 原子提交到本地 SSD cache，Python 校验后再复制到 slot，因此还额外产生一次本地 write/read；论文不以理想 in-process 路径或其他服务器的 GDR 数据替代 Spark 结果。
...
第一版只适配 vLLM 0.25.1。现有仓库已经实现 Foreground-QoS 控制面、Level-0 `mode=keep`、Sleep API 管理、OpenAI proxy、SQLite event log、PySide6/Vue UI、HiberCache 配置、能力探测和 host-staged RDMA 微基准。前三轮分别完成 header-only expert catalog/trace schema、route-history/conformal planner，以及 controller/API/UI。随后新增的数据面代码将 safetensors expert 描述扩展为可 `pread` 的 shard/file offset；把 Marlin 转换后的每个 expert 保存为带模型指纹、tensor layout 和 SHA-256 的原子 `.pllmex` 对象；弹性启动时只为每层分配 `K` 个 ModelOpt NVFP4 physical rows，并在 Marlin modular kernel 前根据 actual Top-22 同步加载和发布 `expert_map`。另一个通用 live-state store 以 64MiB chunks、component/chunk SHA-256 和 manifest-last 协议承载 SSD/RDMA transaction，但 NemotronH Mamba/KV/RNG serializer 尚未接入。该代码尚未启动 vLLM、编译 RDMA bridge 或运行 GPU 验证，因此状态是 **implemented but untested**，不是运行结果。

以下表格区分已验证 control plane、已实现未运行的数据面和未来工作：

| 组件 | 当前状态 | 完成证据 |
| --- | --- | --- |
| expert catalog 与 trace schema | header parser/schema 已实现 | 20,480 objects、schema unit tests |
...
| resource-envelope planner 与控制 API | recommendation-only 已实现 | UMA/独显容量域测试、API 测试、证据标签 |
| NVFP4 expert slot manager | 代码已实现，未运行 | slot remap kernel test、checksum fault injection |
| per-expert SSD loader | `.pllmex`/atomic manifest 已实现，未运行 | 20,480-object export、first-miss latency |
| per-expert RDMA loader | RC QP get/put 已实现，未编译/未双机运行 | checksum、network throughput、failure injection |
| live-state SSD/RDMA carrier | 分块事务协议已实现，未运行 | corruption、manifest-last、remote fetch tests |
| transactional Mamba serializer | 未实现 | 每 token pause 后 greedy token equality |
| full SparkLoad | loader 配置完成，未测 75GiB | cold/warm restore 原始数据 |
...
动态容量变化要求 `--enforce-eager`。PLLM 先用 vLLM Level 0 在 token boundary quiesce，然后撤销 mapping、释放旧 layer parameters、按 32/64/128/256/512 profile 重建 tensors、从 backing store 恢复 retained experts 并重建 Marlin kernel。40 层 resize 目前是 fail-closed、非跨层原子事务：中途失败会保持 vLLM quiesced，需要重试或重启。首次使用前还必须完整加载一次 512-expert 模型，将约 59GiB routed runtime rows 导出至 `/mnt/ssd-storage/pllm-experts`；这是额外 SSD 占用，而不是免费的 checkpoint view。

远端 tier 不再使用 `ib_write_bw` 代替数据传输。`pllm-rdma-store` 用 TCP 控制面交换 RC QP 与 MR，执行 checksummed expert package 的 get/put 和远端落盘确认。当前 bridge 是独立进程：远端 bytes 进入 registered host buffer 后先原子填充本地 SSD cache，Python 再校验并复制到 CUDA/UMA slot，因此比 in-process pinned-buffer 路径多一次本地 write/read；论文仍不声明 GPUDirect RDMA。控制器在 DGX Spark 上用 system `MemTotal` 构造 coherent-UMA capacity，在独显上使用 NVML VRAM total。

## 5. 评估设计

### 5.1 平台与基线

GPU 空闲后在 DGX Spark 上使用 Nemotron NVFP4；双机实验使用第二台 Spark 或明确标注的 100Gbps 主机。前台 workload 包括 Blender Benchmark、可重复 graphics trace、NVENC encode 和 NVMe media trace。基线为：
...
第三轮 dry-run 服务在不加载模型时读取实时 NVML/系统指标，并通过 `GET /api/v1/expert-residency` 与 `POST /api/v1/expert-residency/plan` 输出建议。数据面实现后又新增 deferred tests，但根据当前显存约束没有运行 pytest、CMake、vLLM export 或 RDMA server/client。故最后可引用的回归结果仍是数据面修改前的 39 项；新代码不能继承该绿灯。只有 vLLM Unix runtime 返回 40 层全部注册，API 才允许把 `data_plane_ready` 和 `executable` 置为 true。
...
4. vLLM fused NVFP4/Marlin slot remap 已实现但未运行；它固定 vLLM 0.25.1、ModelOpt NVFP4、Marlin modular backend 和 eager mode。任一版本或 backend 不匹配都会拒绝启动。
...
7. 40 层 destructive resize 尚非原子事务；任一层失败会设置 `faulted=true`、撤销 ready 状态并让服务保持 quiesced。自动 resize 默认关闭。真实模型实验未完成，论文不应声称系统有效。
