...
> 研究设计与系统原型稿。本文区分 RTX PRO 实测、synthetic control plane 与 DGX Spark 待验证结果。当前已完成真实 Nemotron/EER、Level 1/2、CUDA admission 和同机 RC RDMA 验证；真实 predictor、Mamba exact resume、独立双机吞吐与 DGX Spark UMA 尚未完成。
...
在 RTX PRO 6000 96GB 上，128-slot EER 能完成真实生成；PLLM Level 2 在 0.185s 内回收 44.17GiB，使原本 OOM 的 60GiB 前台 allocation 成功，但本地恢复仍需 41.39s。实验同时暴露一个关键负面结果：fused Top-22 要求 `22B<=128`，故精确路径将 batched tokens 限制为 5，首个短请求耗时 63.94s。当前结果证明容量释放可行，不证明 EER 已达到可用吞吐或事务式逐 token 等价。
...
| per-expert RDMA loader | LIVE object path | 20MiB RC PUT/GET；远端推理集成待测 |
...
真实验证已完成 `55 passed`、CMake、完整 expert export、128-slot vLLM、Level 1/2、60GiB CUDA admission 和 RC RDMA PUT/GET。vLLM Unix runtime 返回 40 层注册、20,480 objects 与 `data_plane_ready=true`；前端单独显示实际 128 slots 和 planner 建议 256 slots。Level 0 同一 stream 在暂停期间无新 chunk 且无需重连，但 independent no-pause requests 也不能 bitwise deterministic，因此 transactional exact resume 仍未建立。
