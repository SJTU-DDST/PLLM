# HiberFlow-EER：前台感知 MoE 驻留与休眠的数据面原型及可行性边界

**HiberFlow-EER: A Measurement-Grounded Prototype for Foreground-Aware MoE Residency and Hibernation**

> 研究设计与系统原型稿，硬件证据基线更新至 2026-07-19。本文区分 RTX PRO 实测、synthetic control plane 与 DGX Spark 待验证结果。当前已完成真实 Nemotron/EER、Level 1/2、CUDA admission、75→71 跨机一侧 RDMA 内存池、GNOME 前台识别和 LongBench QA 全驻留基线；真实 predictor、Mamba exact resume、Blender QoS 对照与 DGX Spark UMA 尚未完成。
...
桌面级 AI 终端需要让一个后台大模型与游戏、Blender、视频编码等前台任务共享容量、内存带宽、算力和功耗。现有系统通常在两个极端间选择：保持模型完整驻留，或者暂停并卸载整个模型。前者可能阻止前台任务分配资源，后者对 75GiB 级模型产生数秒到数十秒恢复延迟。MoE 模型的稀疏激活提供了中间状态，但统一内存机器上的 GPU-to-CPU expert offload 不一定释放物理容量。本文报告一个在 RTX PRO 6000 上实测、面向 DGX Spark 迁移的研究原型，而不是已经完成的 DGX Spark 系统。

本文提出 HiberFlow-EER，一种保持原始 MoE Top-k 路由语义的前台感知运行时原型。系统把前台需求表示为随时间变化的资源包络，并在完整驻留、弹性专家驻留、token-boundary yield 和深度休眠间选择。弹性阶段仅保留部分 routed experts；基于路由历史和中间表示预测后续工作集，从本地 NVMe 或 ConnectX-7 远端内存预取。实际 router 结果始终具有权威性：预测 miss 必须等待正确 expert，不允许用预测 expert 替代。目标设计使用在线校准的 prediction set 联合选择 expert cache、预取范围、I/O 上限和 decode duty cycle；当前校准器只有 synthetic trace 证据，真实 Nemotron predictor 尚未完成。

深度休眠阶段的目标协议是在 token 边界提交 Attention KV、Mamba recurrent state、采样器和输出 ledger，直接丢弃已有 canonical checkpoint 的不可变权重，并从 NVMe 或无 GPUDirect RDMA 的 host-staged RDMA 路径恢复。当前实现只有 live-state byte carrier 与 manifest transaction，尚未接入 NemotronH serializer，因此不把 exact resume 列为实测结果。本文实现了只读取 17 个 safetensors header 的 catalog；它确认 74.783GiB tensor 中严格匹配 `experts.<id>` 的 physical expert objects 为 59.063GiB，非 routed 权重为 15.720GiB。
...
实验同时暴露三个关键负面结果：fused Top-22 要求 `22B<=128`，故精确路径将 batched tokens 限制为 5；EER-256 在 LongBench 首条 MQA 的 499s 删失窗口内仍未完成，发生 124,287 次 miss 并换入 358.43GiB，延迟下界超过全驻留同请求 226.09 倍；全 512 专家与 Sleep Mode 的组合在 Marlin repack 阶段启动 OOM。全驻留原生 vLLM 则完成 MQA/NQA/TQA 各 50 条，F1 为 0.3600、总吞吐 6,734.30 tok/s。当前结果证明容量 admission 与远端 profile 搬运可行，同时否证当前 EER 配置对长上下文具有可用吞吐；它不证明 Blender QoS 已改善或事务式逐 token 等价。
...
| **Validated result** | R3 | 15.859GB profile 可通过 4-QP remote-memory pool 跨机 PUT/GET | 已直接 reload 到 GPU slot，或 v2 已跨机加速 |
...
本文的当前闭环结果是**容量 admission 与远端 profile 搬运**。弹性 Pareto 区间、Blender QoS、事务连续性和 DGX Spark UMA 都仍是待验证假设。
...
2. **保持路由语义的专家驻留与风险校准候选机制。** actual Top-22 是唯一权威输入，miss 只导致正确加载和 stall。prediction-set calibration 已在 synthetic request split 上实现，但真实 route accuracy 和边际覆盖仍是待验证算法假设。
...
4. **可证伪的 phase-boundary 设计。** 中等压力下保留 live state 并逐层换 expert；高压力下应提交 Attention/Mamba/sampler/token ledger 后丢弃不可变权重。当前实现验证了边界两侧的数据 carrier 和资源动作，但 exactly-once stream 仍是未完成协议，不列为结果。
...
Attention KV 是 append-only block，适合持续 shadow flush；Mamba state 每 token 整体更新，系统在复制当前约 162MiB recurrent state 与从较旧 checkpoint teacher-force replay suffix 间选择。只有 object manifest、checksum 和 commit record durable 后，系统才应释放 KV、expert slots、dense weights 和 workspace。当前代码完成了通用 bytes/components transaction 与 manifest-last carrier，但 `serializer_attached=false`；Level 2 实测依赖 vLLM 原生 drop/reload，不等价于上述完整提交协议。
...
`T_wall` 是用户可见主指标。`T_RDMA` 只统计 verbs post 到 completion，用于判断网络是否瓶颈；PUT 的 `T_source` 是本地 `.pllmex` read，GET 的 `T_sink` 是 75 本地文件 write。多 worker 的 `sum(bytes)/max(worker phase)` 仅在 phase 高度重叠时近似 aggregate throughput，因此只能作为 diagnostic，并与 wall throughput 同时报告。
...
| --- | --- | --- | --- |
| H1 | **当前 128/256 配置被否证** | EER-256 首条 MQA >499s、>226.09x，换入 358.43GiB；full 512 + Sleep 启动 OOM | 新 kernel/pipeline 后重扫 slots、byte hit 与前台 QoS--TPOT--restore Pareto curve |
| H2 | **synthetic only** | request-split predictor 和 conformal set 可运行；反例中 domain shift 使 coverage 从 1.0 降至 0 | 真实 Nemotron route trace，并与 popularity/top-n/LRU 对照 |
| H3 | **尚未实验** | I/O 与 decode governor API 存在 | Blender 三基线因果对照，同时记录 render throughput、LLM TPOT 和带宽 |
...
真实验证已完成 `69 passed`、CMake、完整 expert export、128-slot vLLM、Level 1/2、60GiB CUDA admission、RC RDMA object path 与跨机 volatile pool。vLLM Unix runtime 返回 40 层注册、20,480 objects 与 `data_plane_ready=true`；前端单独显示实际 128 slots 和 planner 建议 256 slots。Level 0 同一 stream 在暂停期间无新 chunk 且无需重连，但 independent no-pause requests 也不能 bitwise deterministic，因此 transactional exact resume 仍未建立。
...
未完成预测记为错误答案，而把 F1 标为 N/A；它说明控制器必须在观测到 miss
debt 超预算后退出 paging，当前静态 planner 尚未实现这个在线闭环。
...
| RDMA pool v1 PUT | 15,858,978,307B runtime experts | 2.545s | 71 volatile host MR，不落盘 | 49.85Gb/s end-to-end profile offload |
| RDMA pool v1 GET | 15,858,978,307B runtime experts | 4.012s | 75 本地 `.pllmex` 文件 | 31.62Gb/s end-to-end profile reload carrier，尚未入 GPU slot |
| RDMA pool v2 smoke | 124MB 合成 payload | 纯 verbs 约 94.0/79.1Gb/s | 75 本机 RoCE loopback | 只验证优化协议功能，不作为跨机结果 |

因此，本文既不声称 RDMA pool 已将 Level 2 restore 从 41.39s 缩短到 2.545s，也不声称 15.859GB profile 等于完整 74.8GiB checkpoint。直接 GPU/UMA slot reload 与相同 profile 的 NVMe baseline 完成后，才能计算 source speedup 和 resume speedup。
...
| R3 cross-host remote pool | `results/rdma_memory_pool_live_{put,get}_sharded.json` | `pllm-rdma-pool` 4-shard PUT/GET |
| predictor 反例 | `results/expert_residency_simulation.json` | synthetic trace replay |
...
证据快照以 commit `24a7dc6` 和上述原始 JSON 为准；后续代码优化必须产生新的独立 artifact，不覆写 v1 实测。
...
HiberFlow-EER 当前是一个 measurement-grounded prototype。它已证明 120B NVFP4 模型可以在 128-slot route-preserving Marlin 下功能性运行、Level 2 能快速恢复前台容量 admission、真实 15.859GB expert profile 能进入远端 volatile memory pool。LongBench 结果同时否证了当前 128/256-slot 实现具有可用长上下文吞吐：EER-256 首条请求的延迟下界已超过 full-resident 226.09 倍。系统尚未证明中等压力下存在优于 full/hibernate 的 EER 区间，也未完成 exact live-state resume 或 DGX Spark UMA 实验。潜在研究贡献必须转化为能在 miss-debt 越界前退出 paging 的在线 phase-boundary 算法，并由 Blender/游戏因果对照、真实 routes、新 expert kernel、direct slot reload 和事务连续性实验验证。
