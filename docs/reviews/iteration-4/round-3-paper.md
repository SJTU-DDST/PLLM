...
> 研究设计与系统原型稿，硬件证据基线对应 PLLM commit `24a7dc6`；本轮论文与测试修订未产生新性能数据。本文区分 RTX PRO 实测、synthetic control plane 与 DGX Spark 待验证结果。当前已完成真实 Nemotron/EER、Level 1/2、CUDA admission、75→71 跨机一侧 RDMA 内存池和 GNOME 前台识别；真实 predictor、Mamba exact resume、Blender QoS 对照与 DGX Spark UMA 尚未完成。
...
截至证据版本 `24a7dc6`，假设状态为：

| 假设 | 当前状态 | 现有证据 | 仍缺少的决定性实验 |
| --- | --- | --- | --- |
| H1 | **尚未建立** | 128-slot 可功能性生成，但首请求 63.94s | 32/64/128/256/full 的前台 QoS--TPOT--restore Pareto curve |
...
真实验证已完成 `63 passed`、CMake、完整 expert export、128-slot vLLM、Level 1/2、60GiB CUDA admission、RC RDMA object path 与跨机 volatile pool。vLLM Unix runtime 返回 40 层注册、20,480 objects 与 `data_plane_ready=true`；前端单独显示实际 128 slots 和 planner 建议 256 slots。Level 0 同一 stream 在暂停期间无新 chunk 且无需重连，但 independent no-pause requests 也不能 bitwise deterministic，因此 transactional exact resume 仍未建立。
...
HiberFlow-EER 当前是一个 measurement-grounded prototype。它已证明 120B NVFP4 模型可以在 128-slot route-preserving Marlin 下功能性运行、Level 2 能快速恢复前台容量 admission、真实 15.859GB expert profile 能进入远端 volatile memory pool。它尚未证明中等压力下的 EER 优于 full/hibernate，也未完成 exact live-state resume 或 DGX Spark UMA 实验。潜在研究贡献是前台 SLO 驱动的动态资源包络、风险校准 route-preserving residency 与 hibernation phase boundary；是否成立必须由 Blender/游戏因果对照、真实 routes、direct slot reload 和事务连续性实验决定。
