...
| **Validated result** | R2 | 128-slot exact-route Marlin 能生成并通过 Level 1/2 恢复 | EER 吞吐可用或优于 full residency |
...
真实验证已完成 `61 passed`（另有一个旧测试把无效 `XDG_RUNTIME_DIR` 固定期望为 `/tmp`，与实现随后尝试 `/run/user/$UID` 的既有语义冲突，故未作为本轮通过项）、CMake、完整 expert export、128-slot vLLM、Level 1/2、60GiB CUDA admission、RC RDMA object path 与跨机 volatile pool。vLLM Unix runtime 返回 40 层注册、20,480 objects 与 `data_plane_ready=true`；前端单独显示实际 128 slots 和 planner 建议 256 slots。Level 0 同一 stream 在暂停期间无新 chunk 且无需重连，但 independent no-pause requests 也不能 bitwise deterministic，因此 transactional exact resume 仍未建立。
...
HiberFlow-EER 当前是一个 measurement-grounded prototype。它已证明 120B NVFP4 模型可以在 128-slot exact-route Marlin 下运行、Level 2 能快速恢复前台容量 admission、真实 15.859GB expert profile 能进入远端 volatile memory pool。它尚未证明中等压力下的 EER 优于 full/hibernate，也未完成 exact live-state resume 或 DGX Spark UMA 实验。潜在研究贡献是前台 SLO 驱动的动态资源包络、风险校准 exact residency 与 hibernation phase boundary；是否成立必须由 Blender/游戏因果对照、真实 routes、direct slot reload 和事务连续性实验决定。
