...
export PLLM_VLLM_GPU_MEMORY_UTILIZATION="${PLLM_VLLM_GPU_MEMORY_UTILIZATION:-0.45}"
# A fused MoE call cannot replace slots mid-kernel. This bound guarantees
# enough physical slots for exact Top-22 routing across the whole microbatch.
export PLLM_VLLM_MAX_BATCHED_TOKENS="${PLLM_VLLM_MAX_BATCHED_TOKENS:-$((PLLM_EER_SLOTS_PER_LAYER / 22))}"
